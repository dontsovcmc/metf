...
| Component | Version | Where the cited lines live |
|---|---|---|
| METF firmware | protocol 7 | `src/main.cpp` |
| ESP32 Arduino core | 3.2.0 (pioarduino `platform-espressif32` 54.03.20) | `framework-arduinoespressif32@src-*/libraries/WiFi/src/` |
| ESP8266 Arduino core | 3.1.2 (`espressif8266@4.2.1`) | `framework-arduinoespressif8266/libraries/` |
| AsyncTCP | 3.3.2 | `.pio/libdeps/esp32-c6-super-mini/AsyncTCP/src/` |
| ESPAsyncWebServer | 3.3.15 (ESP32), 1.2.3 (ESP8266) | `.pio/libdeps/<env>/ESP*Async*Web*/` |
...
*Enforced:* `wifi_watch()` (`src/main.cpp:95`), `wifi_note_down()`
(`src/main.cpp:65`), `wifi_note_up()` (`src/main.cpp:80`).
...
(`AsyncServer::begin`, `AsyncTCP.cpp:1551`).
...
*Enforced:* `src/main.cpp:275` (log and continue), `src/main.cpp:826`.
...
*Evidence:* `_autoReconnect(true)` by default (`STA.cpp:231`); on
`ARDUINO_EVENT_WIFI_STA_DISCONNECTED` the core calls `disconnect()` then
`connect()` when the reason is reconnectable (`STA.cpp:150-165`); the list of
such reasons includes `NO_AP_FOUND`, `BEACON_TIMEOUT`, `AUTH_EXPIRE` and
`4WAY_HANDSHAKE_TIMEOUT` (`STA.cpp:58-84`).

*Limits, and they are real:* `WIFI_REASON_ASSOC_LEAVE` is excluded - a voluntary
`WiFi.disconnect()` is never followed by a reconnect (`STA.cpp:151`), which is
...
(`WiFiSTA.h:68`). Since the server starts either way (P2), the only thing the
...
function that uses them inside the callbacks"* (ESPAsyncWebServer `README.md`,
...
(`src/main.cpp:399-401` and the handler above it).
*Cost of breaking it:* AsyncTCP discards poll events once its queue passes three
quarters of `CONFIG_ASYNC_TCP_QUEUE_SIZE` = 64 (`AsyncTCP.cpp:205`, with the
library's own comment about callbacks "starving other connections"); an accepted
client has 3 s to send its request (`WebServer.cpp:56`); unacknowledged data
times out after `CONFIG_ASYNC_TCP_MAX_ACK_TIME` = 5000 ms (`AsyncTCP.h:76`).

### P6. Responsiveness beats power

`WiFi.setSleep(false)` (`src/main.cpp:287`). A dozing station answers on the
...
`src/main.cpp:53`), carrying the outage length and the attempt count so the
...
Both event subscriptions are registered in `wifi_watch()` (`src/main.cpp:95`):
...
|---|---|---|---|---|
| 1 | Async handlers run in the shared `async_tcp` task, and `delay()` there blocks every connection | a 4 s button press killed all HTTP for 4 s | `/pulse` arms a `Ticker`, answers via `RESPONSE_TRY_AGAIN` | library README; `src/main.cpp:399` |
| 2 | AsyncTCP drops poll events when its queue fills | silent loss of responses under a blocked task | keep the task free (P5) | `AsyncTCP.cpp:205`, queue 64 |
| 3 | An accepted client has 3 s to send its request | harness requests during a blocked task were refused, not queued | same | `WebServer.cpp:56` |
| 4 | Unacked data times out at 5 s | a 4 s block left 1 s of margin | same | `AsyncTCP.h:76` |
| 5 | `tcp_poll` fires every ~500 ms (interval 1) | a deferred answer can lag the event it reports by up to half a second | documented as a protocol property of `/pulse` | `AsyncTCP.cpp:68`; [api.md](api.md) |
| 6 | ESP8266 `Ticker::once_ms` runs the callback in SYS context | GPIO work from SYS context is a needless risk | ESP8266 uses `once_ms_scheduled()`, which runs it from `loop()` | `Ticker.h`, the comments above both methods; `src/main.cpp:399` |
| 7 | ESP32 `Ticker` dispatches from the `esp_timer` task | no SYS-context concern, no scheduled variant needed | `once_ms()` | `Ticker.cpp`, `dispatch_method = ESP_TIMER_TASK` |
| 8 | `waitForConnectResult()` defaults to 60 s and is not uniform: 2.9 s with no AP, the full 60 s with a wrong password | a minute of no HTTP after a bad flash | 15 s, and the server starts regardless | `WiFiSTA.h:68`; `src/main.cpp:58` |
| 9 | The core reconnects by itself for reconnectable reasons | a hand-written retry loop would race with it | no such loop exists | `STA.cpp:231`, `STA.cpp:150-165`, `STA.cpp:58-84` |
| 10 | `ASSOC_LEAVE` - a voluntary `disconnect()` - is never auto-reconnected | a stray `WiFi.disconnect()` would park the board forever | the only `disconnect()` is immediately followed by `begin()` | `STA.cpp:151`; `src/main.cpp:272-274` |
| 11 | `AUTH_FAIL` is not in the reconnectable list | with a wrong password some APs make the core stop retrying | not handled - see [Known gaps](#known-gaps-and-non-goals) | `STA.cpp:58-84`, `STA.cpp:140` |
| 12 | Modem sleep is on by default; replies wait for the DTIM beacon | ping 6 → 260 ms, drops on a churned network | `WiFi.setSleep(false)` | `src/main.cpp:287`; [architecture.md](architecture.md) |
| 13 | `AsyncServer::begin()` binds `IP_ANY` and needs no address | is what makes P2 possible at all | server starts before any IP exists | `AsyncTCP.cpp:1551` |
| 14 | DHCP may hand out a different address after an outage | the harness addresses the board by IP | the address is pinned by MAC on the router, and the address that came back is printed on return | `wifi_note_up()`, `src/main.cpp:80` |
| 15 | `millis()` wraps after 49 days | a board that lives longer than that | the throttle compares unsigned differences, never absolute values | `src/main.cpp:65-79` |
| 16 | ESP8266 event subscriptions die with the returned handler object | the ESP8266 build would log nothing | the handlers are globals | `src/main.cpp:89-92` |
...
1. **`AUTH_FAIL` stops the retries.** `STA.cpp:58-84` does not list
...
4. **ESP8266 is unverified.** The environment builds and the event handlers are
   ...
   `ESP8266WiFiSTA.cpp:443`) and its default was not confirmed from source.
