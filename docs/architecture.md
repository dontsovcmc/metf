...
| `POST /pulse` | a waveform: JSON body with a line per pin - first level, segment durations, and an `at_ms` offset for simultaneity - or the older form body for one level over `duration_ms`; timed by the ESP on a `Ticker` per line, answered at once with `202` and the batch length; 8 pins at once, `409` for a pin already pulsing |
| `GET /pulse/stat` | what the last batch really did by the board's clock: the ordered segments and the moment of every edge, release included |
...
- **Wait by arming a timer, and answer at once.** `/pulse` is the worked example: it arms a `Ticker` per line (in the core of both platforms - no extra library), answers `202` with the batch length, and returns. The timer re-arms itself for the next segment, so a whole waveform costs one request. On ESP8266 the timer uses `once_ms_scheduled()`, which runs the callback from `loop()` instead of SYS context; ESP32 has no such variant and its `Ticker` already dispatches from the `esp_timer` task.
...
- **Time the intervals on the board, and report them back.** Handing the clock to the client (protocol 8) fixed the deferred answer but left every gap *between* edges measured across Wi-Fi: on a weak link a gap ordered as 0.3 s arrived as 2 s, and the bench read that as a defect in the device under test. Protocol 14 takes the whole waveform in one request and answers the question "what did you actually put on the pin" with `GET /pulse/stat`. A bench asserts from that answer, never from what it ordered.
