...
public:
    ...
    */
    ...
    struct Stat
    {
        uint32_t requests = 0;   // сколько пакетов пришло
        uint32_t replies = 0;    // на сколько ответили
        uint32_t dropped = 0;    // сколько выбросили: молчим или время не задано
        uint32_t ignored = 0;    // сколько не похожи на запрос клиента
        uint32_t last_epoch = 0; // время в последнем ответе
