...
private:
    ...
    */
    ...
    void note_self_down();          // запомнить, что связь рвём мы
    bool ours_now() const;          // событие разрыва - эхо нашей команды?
    void note_up();
    ...
    /*
    Момент, когда связь рвали мы сами.

    Перед каждой попыткой прошивка обрывает предыдущую, и ядро отвечает на это
    событием с кодом 8 (ASSOC_LEAVE) - тем же, каким прощается уходящий на
    перезагрузку роутер. Записать его в last_reason_ значит соврать человеку о
    причине.

    Отличаем по времени, а не по флагу «ждём одно событие»: когда связи уже
    нет, наш разрыв не рождает события вовсе, и взведённый флаг съел бы
    настоящую причину следующей неудачи. Так и было: плата 137 секунд без сети,
    десять неудачных попыток - и last_reason 0. Своё событие приходит
    мгновенно, чужое - через секунды.
    */
    std::atomic<uint32_t> self_down_at_{0};
    std::atomic<bool> self_down_armed_{false};
