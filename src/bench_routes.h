...
private:
    ...
    */
    ...
    struct PulseSlot {
        /*
        Два таймера на линию, по очереди, и это не роскошь. Переставить Ticker
        из его собственного колбэка нельзя: `Ticker::_attach_us` удаляет свой
        же esp_timer и присваивает новую `std::function` поверх той, которую в
        этот момент исполняет, - проверено на плате, она уходит в перезагрузку
        посреди пачки. Следующий участок поэтому заводится на соседнем таймере:
        он заведомо не работает, его колбэк давно вернулся.
        */
        Ticker timer[2];
        uint8_t turn = 0;
