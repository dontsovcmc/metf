...
AP_WAIT_S = 40.0          # от команды до поднятой точки
SCAN_WAIT_S = 45.0        # сколько сканируем эфир, прежде чем сдаться
CHANNEL_WAIT_S = 90.0     # сколько ждём, пока канал в эфире и канал платы сойдутся
...
def _ap_down(metf):
    """
    Погасить точку в конце модуля.

    Точка, поднятая по просьбе, живёт десять минут, и следующий прогон начался
    бы с чужого хвоста. Гашение - единственное, что здесь общее на модуль:
    подъём и вход проверяются перед каждым тестом (см. `ap` и `phone`).
    """
    yield
...
def _raise_ap(metf) -> dict:
    """Попросить точку и дождаться её."""
    assert metf.command('/wifi', action='ap').status == 202
    try:
        return metf.wait_until(lambda w: w['ap_up'], AP_WAIT_S, what='точка поднята')
    except AssertionError as first:
        # Один раз за много прогонов точка не поднялась за 20 с, хотя команду
        # плата приняла и об ошибке железа не сообщила; вручную она встаёт
        # меньше чем за секунду (10 из 10). Повтор отделяет «медленно» от
        # «отказала»: если и со второй просьбы не встала - дело в плате.
        print(f'точка не поднялась с первой просьбы: {first}')
        assert metf.command('/wifi', action='ap').status == 202
        return metf.wait_until(lambda w: w['ap_up'], AP_WAIT_S,
                               what='точка поднята со второй просьбы')


@pytest.fixture
def ap(metf, _ap_down) -> str:
    """
    Поднятая точка доступа - перед каждым тестом.

    Обстановка проверяется, а не предполагается: тест смены сети уводит плату
    в сеть стенда, и её точка после этого гаснет. Общая на модуль точка делала
    бы всё, что идёт следом, зависимым от порядка. Проверка стоит одного
    запроса, подъём - секунды, и только когда он действительно нужен.
    """
    state = metf.wifi()
    if not state['ap_up']:
        state = _raise_ap(metf)
...
def phone(atboard: AtBoard, ap: str) -> Phone:
    ...
    Вход тоже проверяется перед каждым тестом. Общий на модуль он был дешевле,
    но делал набор зависимым от порядка: тест смены сети уводит телефон с
    точки (иначе плата считает, что человек настраивает, и не пробует сеть) -
    и всё, что пошло бы после него, осталось бы без телефона. Доказано
    прогоном набора задом наперёд: 6 падений из 8. В обычном случае это одна
    AT-команда, вход делается только когда телефона в точке нет.
    """
    if atboard.joined_to() != ap:
        try:
...
def test_точка_видна_в_эфире(atboard: AtBoard, ap: str, metf) -> None:
    ...
    # Канал сверяем не разовым снимком, а до схождения - и обе стороны берём
    # свежими. Радио у платы одно на точку и станцию: пока идёт скан (плата
    # раз в минуту пробует сеть), оно ходит по каналам, и /wifi отдаёт тот, на
    # котором радио оказалось в этот миг, - отсюда и расхождение вида 4 против
    # 11. Ответ платы берём только с несканирующего радио, а список сетей
    # пересканируем: он тоже мог устареть. Точка, застрявшая на чужом канале,
    # этой проверки не пройдёт - расхождение у неё не рассасывается.
    deadline = time.time() + CHANNEL_WAIT_S
    while True:
        state = metf.wait_until(lambda w: not w['scanning'], SCAN_WAIT_S,
                                what='радио платы перестало сканировать')
        if seen[ap]['channel'] == state['channel']:
            break
        assert time.time() < deadline, (
            f'точка в эфире на канале {seen[ap]["channel"]}, а своим плата '
            f'считает {state["channel"]} - точка осталась на старом канале')
        time.sleep(3.0)
        with contextlib.suppress(AtError, KeyError):
            seen[ap] = {net['ssid']: net for net in atboard.scan()}[ap]
...
@pytest.mark.slow
def test_смена_сети_через_портал(phone: Phone, atboard: AtBoard, metf, network) -> None:
    """
    ...
    Телефон с точки уводим, как только увидели новый адрес, - и не для
    красоты. Клиент на точке значит для платы «человек настраивает сеть»: пока
    он там, точка стоит, а проб сети нет десять минут. Тест, забывший выйти,
    ждёт возвращения, которого не будет, и обвиняет прошивку в том, что она
    делает правильно. Ровно это и случилось: после forget плата ушла в свою
    точку и осталась в ней, потому что «человек» никуда не делся.
    """
    ...
    atboard.leave()          # человек увидел адрес и закрыл страницу
